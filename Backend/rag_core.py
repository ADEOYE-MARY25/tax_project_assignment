...
""")
    ...
    def _build_graph(self):
        ...
        def multilingual_node(state: MessagesState):
            ...
            if any(word in user_question for word in ["yoruba", "èdè yorùbá", "yorùbá"]):
                target = "yoruba"
            elif any(word in user_question for word in ["pidgin", "naija"]):
                target = "pidgin"
            elif "hausa" in user_question:
                target = "hausa"
            elif "igbo" in user_question:
                target = "igbo"
...
# etc.
            ...
            # Call multilingual tool with detected target
...
#         @tool
#         def multilingual_output_node(text: str) -> str:
#             """
#     Produce multilingual summaries without altering legal meaning.

#     Use this tool when:
#     - The final answer has already been generated
#     - You want accessibility for Nigerian users
#     - Legal accuracy must be preserved

#     Do NOT use this tool:
#     - Before evidence-based answering
#     - For raw document retrieval
...
#     Output languages:
#     - English (UNCHANGED)
#     - Nigerian Pidgin (Short summary)
#     - Yoruba (Short summary)
#     - Hausa (Short summary)
#     - Igbo (Short summary)

#     Args:
#         final_answer: The legally grounded final answer

#     Returns:
#         Multilingual formatted response
#     """
#             prompt = f"""
# You are a multilingual legal summarizer.

# Text:
# {text}

# Instructions:
# 1. Keep the English version unchanged.
# 2. Provide a SHORT Nigerian Pidgin summary.
# 3. Provide a SHORT Yoruba summary.
...
# # ... inside TaxRAGAgent class ...
#     def run_with_memory(self, question: str, thread_id: str = "default"):
#         """
#         Final version: Clean summarized answer + correct citations
#         """
#         if thread_id not in self.sessions:
#             self.sessions[thread_id] = []
...
#         except Exception as e:
#             print(f"Graph invoke error: {e}")
#             assistant_content = "Sorry, an error occurred while processing your question."
#             citations = []
#         else:
#             # Extract final assistant message
#             assistant_content = "I couldn't find relevant information in the tax documents."
#             citations = []

#             # Find the last AIMessage that is NOT a tool call (i.e., the final response)
#             final_ai_msg = None
...
#             # Collect ALL citations from any ToolMessage in the conversation
#             for msg in result["messages"]:
#                 if isinstance(msg, ToolMessage):
#                     try:
#                         tool_output = json.loads(msg.content)
#                         if isinstance(tool_output, dict) and "citations" in tool_output:
#                             citations.extend(tool_output["citations"])  # Add all
#                     except json.JSONDecodeError:
#                         continue

#             # Deduplicate citations by source_path + page_number (optional, cleaner)
#             seen = set()
#             unique_citations = []
#             for cit in citations:
#                 key = (cit["source_path"], cit.get("page_number"))
#                 if key not in seen:
#                     seen.add(key)
...
#                 assistant_content = "No relevant information was found in the tax documents."
